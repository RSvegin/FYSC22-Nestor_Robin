...
def perform_counts_sum(x, y, region_start, region_stop, left_selection=None, right_selection=None, plotting = 1, printing = 1):
    ...
    suma = peak_region_counts.sum()
    
    if (plotting):
        # these define a region of interest in our data binning:
        vis_region = (region_start*0.98 < x) & (x < region_stop*1.02)
        # now limit our data by 'slicing' it, limiting it to the region of interest:
        
        vis_peak_region_bc     = x[vis_region]
        vis_peak_region_counts = y[vis_region]

        plt.figure()
        color = 'forestgreen'
        
        if (left_selection and right_selection):
            plt.plot(vis_peak_region_bc, vis_peak_region_counts, drawstyle='steps', label='data')
            plt.fill_between(peak_region_bc, peak_region_counts, step="pre", alpha=0.4, label='Summed', color=color)
            plt.vlines(left_selection+right_selection, ymin=0, ymax=peak_region_counts.max()*1.1, label='BG estimation', color='r')  #plot support lines around selected points 
            plt.plot(lin_bc, LineFunc(lin_bc, estimates_lin[0], estimates_lin[1]), color='r', label = 'linear fit', alpha=0.6)  # plot linear fit
        else:
            plt.plot(vis_peak_region_bc, vis_peak_region_counts, drawstyle='steps', label='data')
            plt.fill_between(peak_region_bc, peak_region_counts, step="pre", alpha=0.4, label='Summed', color=color)
    ...
    if (printing):
        print("Adding between {} and {} gives {}".format(region_start, region_stop, suma))
